...
(function () {
 ...
 const copy = {
  mercury: {
   ...
   reference: '반경 2,440 km의 기준구(매끈한 공 모양의 면)를 사용했습니다. 수성의 중력 자료로 계산한 면이 이 기준구보다 얼마나 높거나 낮은지를 보여줍니다.',
  ...
  venus: {
   ...
   reference: '반경 6,051.8 km의 기준구를 그림의 바탕으로 사용했습니다. 원자료의 높이 차이를 이 구 위에 표현한 것이며, 이 반경이 금성의 공식적인 높이 0 기준이라는 뜻은 아닙니다.',
  ...
  moon: {
   ...
   reference: '반경 1,738 km의 기준구(매끈한 공 모양의 면)를 사용했습니다. 원자료가 정한 기준으로, 달의 지오이드가 이 기준구보다 얼마나 높거나 낮은지를 보여줍니다.',
  ...
  mars: {
   ...
   reference: '적도 반경 3,396 km의 기준 타원체를 사용했습니다. 완전한 공이 아니라 위아래가 조금 납작한 모양입니다. 원자료에 담긴 높이 변화를 이 면과 비교해 보여줍니다.',
 ...
 function openGuide(body = state.selected) {
  ...
  dialog.innerHTML = dialogHeader(text.name + ' · 자료 출처', '자료와 그림 설명') + `
   <div class="dialog-copy source-guide" data-source-body="${escape(body)}" data-source-guide-version="1">
    ...
    ${text.reference ? `<p class="source-reference">${escape(text.reference)}</p>` : ''}
