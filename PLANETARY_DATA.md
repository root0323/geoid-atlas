...
이 웹의 수성·금성·화성·달 기준면은 NASA PDS의 실제 수치 자료를 사용합니다. 지형 모델이나 색상 이미지에서 추정한 값이 아닙니다. 지구의 기존 관측·KN18·EGM2008 비교 자료는 수정하지 않았습니다. 지구 3D는 모델 탭 없이 KNGeoid18 지역 자료만 표시하며, 나머지는 회색 기준 타원체로 구분합니다. 지도에는 KN·EGM2008 탭을 유지합니다. [지구 KN 시각화와 적용 범위](EARTH_DATA.md)를 참고하세요. 가상 소행성은 여전히 개념 모형입니다.
...
다음은 수성·금성·화성·달에 대한 설명이며 지구 KN 표시는 [별도 문서](EARTH_DATA.md)를 따릅니다.
